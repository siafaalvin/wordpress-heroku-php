...
function sidebar_menu (){?>
	<ul class="menu-side-canvas"><?php
  if( is_front_page() ) { ?>
    <a href="<?php echo home_url('?term='.$term->slug); ?>" id="cat_<?php echo $term->slug; ?>" ng-click="$event.preventDefault(); cat_post_filter_click($event, 'category', '<?php echo $term->slug; ?>', '<?php echo $term->name; ?>', 'index')">
    </a><?php
  } else { ?>
    <a href="<?php echo home_url('?term='.$term->slug); ?>" id="cat_<?php echo $term->slug; ?>" ng-click="$event.preventDefault(); cat_post_filter_click($event, 'category', '<?php echo $term->slug; ?>', '<?php echo $term->name; ?>', 'index')">
    </a><?php
        ...
        <div class="<?php echo $class; ?>">
            <ul>
                <li class="pro_list_image" data-value="recent_post">
                    <div class="post_cat_img">
                    	<a href="<?php echo home_url(''); ?>" ng-click="$event.preventDefault(); cat_post_filter_click($event, 'recent', 'recent_post', 'Recently Added/Edited', '<?php echo get_stylesheet_directory_uri().'/assets/images/recently_added_edited.jpg'; ?>', 'index')">
                        	<?php get_image('recently_added_edited.jpg', '', '', 'Recently Added Edited');?>
                        </a>
                    </div>
				...
				/*if( scroll_load_post_category_right_sidebar( ) ) {
					$count_posts = wp_count_posts('post');
                    $count = round($count_posts->publish/3);
					for( $i = 0; $i <= $count; $i++ ) {
						foreach( scroll_load_post_category_right_sidebar( ) as $scroll_load_cat ) {
							echo '<li class="pro_list_image">'.$scroll_load_cat.'</li>';
						}
					}
				}*/?>
                ...
                <?php
                    ...
                    if( isset($_GET['task']) && $_GET['task'] == 'edit' || $_GET['task'] == 'new') { ?>
                        <div class="post_new_and_edit_section" >
                            <span class="single_page_pencil_icon_left"><i class="fa fa-edit fa-4x"></i></span>
                            ...
                            <div class="single_edit_action">
                                <ul><?php $url_theme = get_stylesheet_directory_uri(); ?>
                                    ...
                                    <li class="single_edit_icon">
                                        <?php edit_post_link('<img src="'.$url_theme.'/assets/images/pencil_sign.png" width="35" height="35"/>',''); ?>
                                    </li><?php
                                    }
                                    if( isset($_GET['task'] ) ) { ?>
                                    <li class="single_close_icon"><?php
                                    if( isset( $_GET['postid'] ) ) { ?>
                                        <a href="<?php echo get_the_permalink($_GET['postid']); ?>"><?php get_image('close_sign.png', 35, 35);?></a><?php
                                    } else { ?>
                                        <a href="javascript:void(0);"><?php get_image('close_sign.png', 35, 35);?></a><?php
                                    } ?>
                                    </li><?php
                                    } else { ?>
                                        <li class="single_close_icon">
                                        <a href="javascript:void(0);"><?php get_image('close_sign.png', 35, 35);?></a>
                                    </li><?php
                                    } ?>
                                </ul>
                            </div><?php
