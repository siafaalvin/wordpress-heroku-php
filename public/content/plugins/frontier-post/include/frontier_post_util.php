...
function frontier_tax_input($tmp_post_id, $tmp_tax_name, $input_type = 'checkbox', $tmp_selected = array(), $tmp_shortcode_parms, $tmp_tax_list)
	...
	if ( !empty($tmp_tax_name) )
		...
		*/
		...
		wp_dropdown_categories(array('taxonomy' => $tmp_tax_name, 'id'=>$tmp_field_name, 'exclude' => $exclude_list, 'hide_empty' => 0, 'name' => $tmp_input_field_name, 'orderby' => 'name', 'order' => 'DESC', 'selected' => (isset($tmp_selected[0]) ? $tmp_selected[0] : $tmp_selected), 'hierarchical' => true, 'show_count' => false, 'show_option_none' => __("Select", "frontier-post"), 'option_none_value' => '0','class' => 'frontier_post_dropdown cs-select cs-skin-elastic'));
		...
		/*switch ($input_type) 
			...
			case "single":
				...
				wp_dropdown_categories(array('taxonomy' => $tmp_tax_name, 'id'=>$tmp_field_name, 'exclude' => $exclude_list, 'hide_empty' => 0, 'name' => $tmp_input_field_name, 'orderby' => 'name', 'selected' => $tmp_selected[0], 'hierarchical' => true, 'show_count' => false, 'show_option_none' => __("None", "frontier-post"), 'option_none_value' => '0','class' => 'frontier_post_dropdown cs-select cs-skin-elastic')); 
...
Function frontier_post_tax_multi($tmp_cat_list, $tmp_selected, $tmp_name, $tmp_id, $tmp_size)
	{
	$tmp_html = '<select class="frontier_post_dropdown cs-select cs-skin-elastic frontier_post_dropdown" name="'.$tmp_name.'" id="'.$tmp_id.'" multiple="multiple" size="'.$tmp_size.'">';
