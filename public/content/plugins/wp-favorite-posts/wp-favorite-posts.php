...
function wpfp_add_to_usermeta($post_id) {
	...
	$wpfp_favorites_date = wpfp_get_user_meta_for_vote_date();
	//$wpfp_favorites_date[][$post_id] = date("Y-m-d");
	global $wpdb;
	
	date_default_timezone_set('UTC');
	date_default_timezone_get();
	$wpfp_favorites_date_current = date('Y-m-d H:i:s');
	$wpfp_favorites_date[$post_id] = $wpfp_favorites_date_current;
	//$wpfp_favorites_date[$post_id] = date("Y-m-d");
	update_user_meta(wpfp_get_user_id(),'wpfp_favorites_date',$wpfp_favorites_date);
...
function wpfp_link( $return = 0, $action = "", $show_span = 1, $args = array(), $post_id = '' ) {
    ...
    $str .= wpfp_before_link_img($post_id);
    $str .= wpfp_loading_img();
    if ($action == "remove"):
        $str .= wpfp_link_html($post_id, wpfp_get_option('remove_favorite'), "remove", 'remove_from_favorite');
    elseif ($action == "add"):
        $str .= wpfp_link_html($post_id, wpfp_get_option('add_favorite'), "add", 'add_to_favorite');
...
function wpfp_link_html($post_id, $opt, $action, $class) {
    $link = "<a class='wpfp-link ".$class."' href='?wpfpaction=".$action."&amp;postid=". esc_attr($post_id) . "' title='". $opt ."' rel='nofollow'>". $opt ."</a>";
...
function wpfp_before_link_img($post_id) {
...
function wpfp_do_remove_favorite($post_id) {
    ...
    if (is_user_logged_in()) {
		global $wpdb;
		$wpfp_favorites_date = $wpdb->get_var("SELECT NOW()");
		$array[$post_id] = $wpfp_favorites_date;
		$wpfp_favorites_date = wpfp_get_user_meta_for_vote_date();
		$wpfp_favorites_date = array_diff_key($wpfp_favorites_date, $array);
		update_user_meta(wpfp_get_user_id(),'wpfp_favorites_date',$wpfp_favorites_date);
...
function wpfp_get_user_meta_for_vote_date($user = "") {
    if (!empty($user)):
        $userdata = get_user_by( 'login', $user );
        $user_id = $userdata->ID;
        return get_user_meta($user_id, 'wpfp_favorites_date', true);
    else:
        return get_user_meta(wpfp_get_user_id(), 'wpfp_favorites_date', true);
    endif;
}
...
function wpfp_remove_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        ...
        $link = "<a id='rem_$post_id' class='$class' href='?wpfpaction=remove&amp;page=1&amp;postid=". $post_id ."' title='".wpfp_get_option('rem')."' rel='nofollow'>"."<span class=clear_favorites>Remove from Favorites</span>"."</a>";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        $wpfp_options = wpfp_get_options();
        echo "<span class=clear_favorites>".wpfp_before_link_img();
        echo wpfp_loading_img();
        echo "<a class='wpfp-link' href='?wpfpaction=clear' rel='nofollow'>". "Clear All Favorites" . "</a></span>";
    }
}
