	...
	function edit_profile_validate(){
		...
		if(isset($_POST['option']) and sanitize_text_field($_POST['option']) == "afo_user_edit_profile"){
			...
			if(!$error){
				...
				if($this->is_field_enabled('firstname_in_profile') and sanitize_text_field($_POST['first_name'])){
					$userdata['first_name'] = sanitize_text_field($_POST['first_name']);
				}
				
				if($this->is_field_enabled('lastname_in_profile') and sanitize_text_field($_POST['last_name'])){
					$userdata['last_name'] = sanitize_text_field($_POST['last_name']);
				}
				
				if($this->is_field_enabled('displayname_in_profile') and sanitize_text_field($_POST['display_name'])){
					$userdata['display_name'] = sanitize_text_field($_POST['display_name']);
				}
				
				if($this->is_field_enabled('userdescription_in_profile') and sanitize_text_field($_POST['description'])){
					$userdata['description'] = sanitize_text_field($_POST['description']);
				} 
				
				if($this->is_field_enabled('userurl_in_profile') and sanitize_text_field($_POST['user_url'])){
	...
	function update_password_validate(){
		if(isset($_POST['option']) and sanitize_text_field($_POST['option']) == "afo_user_update_password"){
			...
			if(isset($_POST['user_new_password']) and (sanitize_text_field($_POST['user_new_password']) != sanitize_text_field($_POST['user_retype_password']))){
				$msg = __('Your new password dont match with retype password!','wp-register-profile-with-shortcode');
				$error = true;
			}
			...
			if(!$error){
				...
				$_SESSION['reg_error_msg'] = __('Your password updated successfully.','wp-register-profile-with-shortcode');
				$_SESSION['reg_msg_class'] = 'reg_success';
...
function register_widget_pro_afo_shortcode_1( $atts ) {
     global $post;
...
add_shortcode( 'rp_register_contributor', 'register_widget_pro_afo_shortcode_1' );

function register_widget_pro_afo_shortcode_2( $atts ) {
     global $post;
	 extract( shortcode_atts( array(
	      'title' => '',
     ), $atts ) );
...
add_shortcode( 'rp_register_user', 'register_widget_pro_afo_shortcode_2' );
